...
public class AerospikeTemplate implements AerospikeOperations {
	...
	public void save(Object objectToInsert) {
		...
		try {
			...
			final AerospikePersistentEntity<?> entity = mappingContext.getPersistentEntity(objectToInsert.getClass());

			if (entity.hasVersionProperty()) {
				createOrUpdateUsingCas(objectToInsert, key, bins, entity);
			} else {
	...
	private void createOrUpdateUsingCas(Object objectToInsert, Key key, Bin[] bins, AerospikePersistentEntity<?> entity) {
		final ConvertingPropertyAccessor accessor = getPropertyAccessor(entity, objectToInsert);
		...
		Operation[] operations = OperationUtils.operations(bins, b -> Operation.put(b), Operation.get());
